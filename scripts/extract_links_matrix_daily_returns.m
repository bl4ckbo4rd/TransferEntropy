...
%daily geometric returns time series STE histogram, the vertical line
%x=th_STE and the x-axis to that bounded by a benchmark STE histogram from
%the bracketing twenty-one randomised daily geometric returns STE matrices,
%the vertical line x=th_STE and the x-axis (th_ratio). With the th_ratio
%threshold set at r_star, the weight of each link is determined by a
%Fermi-Dirac distribution. The results are collected into a LINKS matrix.

function LINKS = extract_links_matrix_daily_returns(din,w,w_step,m,step,a,clop,r_star)
