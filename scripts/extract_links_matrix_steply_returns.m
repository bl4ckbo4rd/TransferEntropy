...
%th_ratio threshold set at r_star, the weight of each link is determined by a
%Fermi-Dirac distribution. The results are collected into a LINKS matrix.

function LINKS = extract_links_matrix_steply_returns(din,w,w_step,m,step,a,clop,r_star)
...
%plot(x,y1/(bin_width*sum(y1(1:end))),x,y/(bin_width*sum(y(1:end))))
%load(S{11});
%scatter(STE1(:),STEM(:))
