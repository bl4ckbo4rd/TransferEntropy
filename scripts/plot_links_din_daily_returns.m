...
%daily geometric returns STE matrix as a function of time

function plot_links_din_daily_returns(w,w_step,m,step,clop,time_l)
...
for din = din_members
    if clop == 1
        ...
        S2 = strcat('../STE_matrices/close/m=',num2str(m),'/w=',num2str(w),'_step=',num2str(step),'/STE_din=',num2str(din),'_choice=3.mat');
    elseif clop ==2
        ...
        S2 = strcat('../STE_matrices/open/m=',num2str(m),'/w=',num2str(w),'_step=',num2str(step),'/STE_din=',num2str(din),'_choice=3.mat');
    end
    load(S1);
    load(S2);
...
%set(gca,'xticklabel',[])
%tmp = text(oldticksX, zeros(size(oldticksX)), oldticklabels, 'rotation',-90,'horizontalalignment','left'+1);